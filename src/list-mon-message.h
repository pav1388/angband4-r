...
MON_MSG(HIT_HARD,			MSG_GENERIC,	false,	"сильно пострадал[и].")
// MON_MSG(RESIST,				MSG_GENERIC,	false,	"resist[s].")
MON_MSG(RESIST,				MSG_GENERIC,	false,	"сопротивляется.")
// MON_MSG(IMMUNE,				MSG_GENERIC,	false,	"[is|are] immune.")
MON_MSG(IMMUNE,				MSG_GENERIC,	false,	"неуязвим[ы].")
// MON_MSG(RESIST_SOMEWHAT,	MSG_GENERIC,	false,	"resist[s] somewhat.")
MON_MSG(RESIST_SOMEWHAT,	MSG_GENERIC,	false,	"немного сопротивляется.")
...
MON_MSG(FALL_ASLEEP,		MSG_GENERIC,	false,	"засыпает!")
// MON_MSG(WAKES_UP,			MSG_GENERIC,	false,	"wake[s] up.")
MON_MSG(WAKES_UP,			MSG_GENERIC,	false,	"просыпается.")
// MON_MSG(CRINGE_LIGHT,		MSG_GENERIC,	false,	"cringe[s] from the light!")
MON_MSG(CRINGE_LIGHT,		MSG_GENERIC,	false,	"корчится от света!")
// MON_MSG(SHRIVEL_LIGHT,		MSG_KILL,		false,	"shrivel[s] away in the light!")
MON_MSG(SHRIVEL_LIGHT,		MSG_KILL,		false,	"рассыпается на свету!")
// MON_MSG(LOSE_SKIN,			MSG_GENERIC,	false,	"lose[s] some skin!")
MON_MSG(LOSE_SKIN,			MSG_GENERIC,	false,	"теряет немного кожи!")
// MON_MSG(DISSOLVE,			MSG_KILL,		false,	"dissolve[s]!")
MON_MSG(DISSOLVE,			MSG_KILL,		false,	"растворяется!")
// MON_MSG(CATCH_FIRE,			MSG_GENERIC,	false,	"catch[es] fire!")
MON_MSG(CATCH_FIRE,			MSG_GENERIC,	false,	"загорается!")
// MON_MSG(BADLY_FROZEN,		MSG_GENERIC,	false,	"[is|are] badly frozen.")
MON_MSG(BADLY_FROZEN,		MSG_GENERIC,	false,	"сильно замерзает.")
// MON_MSG(SHUDDER,			MSG_GENERIC,	false,	"shudder[s].")
MON_MSG(SHUDDER,			MSG_GENERIC,	false,	"дрожит.")
// MON_MSG(CHANGE,				MSG_GENERIC,	false,	"change[s]!")
MON_MSG(CHANGE,				MSG_GENERIC,	false,	"меняется!")
// MON_MSG(DISAPPEAR,			MSG_GENERIC,	false,	"disappear[s]!")
MON_MSG(DISAPPEAR,			MSG_GENERIC,	false,	"исчезает!")
// MON_MSG(MORE_DAZED,			MSG_GENERIC,	false,	"[is|are] even more stunned.")
MON_MSG(MORE_DAZED,			MSG_GENERIC,	false,	"сильно ошеломл[ён|ены].")
// MON_MSG(DAZED,				MSG_GENERIC,	false,	"[is|are] stunned.")
MON_MSG(DAZED,				MSG_GENERIC,	false,	"ошеломл[ён|ены].")
// MON_MSG(NOT_DAZED,			MSG_GENERIC,	false,	"[is|are] no longer stunned.")
MON_MSG(NOT_DAZED,			MSG_GENERIC,	false,	"больше не ошеломл[ён|ены].")
// MON_MSG(MORE_CONFUSED,		MSG_GENERIC,	false,	"look[s] more confused.")
MON_MSG(MORE_CONFUSED,		MSG_GENERIC,	false,	"выглядит сильно растерянным[и].")
// MON_MSG(CONFUSED,			MSG_GENERIC,	false,	"look[s] confused.")
MON_MSG(CONFUSED,			MSG_GENERIC,	false,	"выглядит растерянным[и].")
// MON_MSG(NOT_CONFUSED,		MSG_GENERIC,	false,	"[is|are] no longer confused.")
MON_MSG(NOT_CONFUSED,		MSG_GENERIC,	false,	"больше не растерян[о].")
// MON_MSG(MORE_SLOWED,		MSG_GENERIC,	false,	"look[s] more slowed.")
MON_MSG(MORE_SLOWED,		MSG_GENERIC,	false,	"выглядит сильно замедленным[и].")
// MON_MSG(SLOWED,				MSG_GENERIC,	false,	"look[s] slowed.")
MON_MSG(SLOWED,				MSG_GENERIC,	false,	"выглядит замедленным[и].")
// MON_MSG(NOT_SLOWED	,		MSG_GENERIC,	false,	"speed[s] up.")
MON_MSG(NOT_SLOWED	,		MSG_GENERIC,	false,	"ускоряется.")
// MON_MSG(MORE_HASTED,		MSG_GENERIC,	false,	"look[s] even faster!")
MON_MSG(MORE_HASTED,		MSG_GENERIC,	false,	"выглядит более быстрым[и]!")
// MON_MSG(HASTED,				MSG_GENERIC,	false,	"start[s] moving faster.")
MON_MSG(HASTED,				MSG_GENERIC,	false,	"начинает двигаться быстрее.")
// MON_MSG(NOT_HASTED,			MSG_GENERIC,	false,	"slow[s] down.")
MON_MSG(NOT_HASTED,			MSG_GENERIC,	false,	"замедляется.")
// MON_MSG(MORE_AFRAID,		MSG_GENERIC,	false,	"look[s] more terrified!")
MON_MSG(MORE_AFRAID,		MSG_GENERIC,	false,	"выглядит сильно испуганным[и]!")
// MON_MSG(FLEE_IN_TERROR,		MSG_FLEE,		false,	"flee[s] in terror!")
MON_MSG(FLEE_IN_TERROR,		MSG_FLEE,		false,	"бежит в страхе!")
// MON_MSG(NOT_AFRAID,			MSG_GENERIC,	false,	"[is|are] no longer afraid.")
MON_MSG(NOT_AFRAID,			MSG_GENERIC,	false,	"больше не боится.")
// MON_MSG(HELD,				MSG_GENERIC,    false,  "[is|are] frozen to the spot!")
MON_MSG(HELD,				MSG_GENERIC,    false,  "замирает на месте!")
// MON_MSG(NOT_HELD,			MSG_GENERIC,    false,  "can move again.")
MON_MSG(NOT_HELD,			MSG_GENERIC,    false,  "снова может двигаться.")
// MON_MSG(DISEN,				MSG_GENERIC,    false,  "seem[s] less magical!")
MON_MSG(DISEN,				MSG_GENERIC,    false,  "выглядит менее магическим[и]!")
// MON_MSG(NOT_DISEN,			MSG_GENERIC,    false,  "seem[s] magical again.")
MON_MSG(NOT_DISEN,			MSG_GENERIC,    false,  "снова выглядит магическим[и].")
// MON_MSG(COMMAND,			MSG_GENERIC,    false,  "falls under your spell!")
MON_MSG(COMMAND,			MSG_GENERIC,    false,  "попадает под ваши чары!")
...
MON_MSG(SHAPE_FAIL,			MSG_GENERIC,    false,  "мерцает на мгновение.")
...
MON_MSG(DISINTEGRATES,		MSG_KILL,		false,	"распадается!")
// MON_MSG(FREEZE_SHATTER,		MSG_KILL,		false,	"freeze[s] and shatter[s]!")
MON_MSG(FREEZE_SHATTER,		MSG_KILL,		false,	"замерзает и рассыпается!")
// MON_MSG(MANA_DRAIN,			MSG_GENERIC,	false,	"lose[s] some mana!")
MON_MSG(MANA_DRAIN,			MSG_GENERIC,	false,	"теряет немного маны!")
// MON_MSG(BRIEF_PUZZLE,		MSG_GENERIC,	false,	"look[s] briefly puzzled.")
MON_MSG(BRIEF_PUZZLE,		MSG_GENERIC,	false,	"выглядит озадаченным[и].")
// MON_MSG(MAINTAIN_SHAPE,		MSG_GENERIC,	false,	"maintain[s] the same shape.")
MON_MSG(MAINTAIN_SHAPE,		MSG_GENERIC,	false,	"сохраняет прежний облик.")
// MON_MSG(UNHARMED,			MSG_GENERIC,	false,	"[is|are] unharmed.")
MON_MSG(UNHARMED,			MSG_GENERIC,	false,	"остался невредимым[и].")
// MON_MSG(APPEAR,			MSG_GENERIC,	false,	"appear[s]!")
MON_MSG(APPEAR,			MSG_GENERIC,	false,	"появляется!")
