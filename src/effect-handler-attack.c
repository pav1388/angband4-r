...
bool effect_handler_EARTHQUAKE(effect_handler_context_t *context)
{
	...
	if (hurt) {
		...
		if (!safe_grids) {
			...
			msg("Вас сильно завалило камнями!%s", dam_text);
		} else {
			...
			switch (randint1(3)) {
				case 1: {
					...
					hurt_msg = "Вы ловко увернулись от взрыва!";
					damage = 0;
					break;
				}
				case 2: {
					// hurt_msg = "You are bashed by rubble!";
					hurt_msg = "Вас завалило камнями!";
					damage = damroll(10, 4);
				...
				case 3: {
					...
					hurt_msg = "Вы зажаты между полом и потолком!";
					damage = damroll(10, 4);
					(void)player_inc_timed(player, TMD_STUN,
						randint1(50), true, true, true);
					break;
	...
	take_hit(player, damage, "землетрясения");
...
bool effect_handler_CURSE(effect_handler_context_t *context)
{
	...
	if (display_dam) {
		...
		strnfmt(note, sizeof(note), " умер! (%d)", dam);
		passed_note = note;
	} else {
		// passed_note = " dies!";
		passed_note = " умер!";
