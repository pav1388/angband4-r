...
static void show_message(struct monster_race_message *msg)
{
	...
	// для русского языка
	// исправления в зависимости от рода монстра для list-mon-message.h
	if (msg->count == 1) { 
		switch (msg->msg_code) {
			...
			case MON_MSG_MORE_CONFUSED:
			case MON_MSG_CONFUSED:
			case MON_MSG_MORE_SLOWED:
			case MON_MSG_SLOWED:
			case MON_MSG_MORE_HASTED:
			case MON_MSG_MORE_AFRAID:
			case MON_MSG_DISEN:
			case MON_MSG_NOT_DISEN:
			case MON_MSG_BRIEF_PUZZLE:
				my_strcat(body, (rf_has(msg->race->flags, RF_FEMALE)) ? "ой." : 
							(rf_has(msg->race->flags, RF_MALE)) ? "ым." : "о.", sizeof(body));
				break;
	...
	if (msg->flags & MON_MSG_FLAG_DAMAGE) {
		if (msg->count <= 1) {
			...
			msgt(msg_type, "%s%s (в среднем %d)", subject, body,
